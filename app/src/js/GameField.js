"use strict";
class GameField {
    constructor(ctx, responsiveManager) {
        this.path = null;
        this.holes = [];
        this.goal = null;
        this.ctx = ctx;
    ...
    isInPath(x, y, radius) {
        return (ctx.isPointInPath(this.path, x - radius, y - radius)
            && ctx.isPointInPath(this.path, x - radius, y + radius)
            && ctx.isPointInPath(this.path, x + radius, y - radius)
            && ctx.isPointInPath(this.path, x + radius, y + radius));
    }
    drawForeground() {
